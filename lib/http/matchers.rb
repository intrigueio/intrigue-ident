    ...
    module Http
      module Matchers
        ...
        def match_http_response_hash(check, hash)
          ...
          generator_match = (hash[:response_body]).to_s.match(%r{<meta name="?generator"? content="?(.*?)"?/>}i)
          generator_string = generator_match.captures.first.gsub('"', '') if generator_match

          # save off the title string
          title_match = (hash[:response_body]).to_s.match(%r{<title>(.*?)</title>}i)
          ...
          set_cookie_header = (hash[:response_headers] || []).flatten.select do |x|
            x =~ /^set-cookie:(.*)/i
          end.join(';').gsub('set-cookie:', '').strip
        ...
        def match_http_response_text(check, http_response_text)
          ...
          # Untested!
          status_string = headers.find { |x| x =~ %r{^HTTP/[\d.]+\s(\d+).*}i; }
          ...
          match = body.match(%r{<title>(.*?)</title>}i)
          title = match.captures.first if match

          match = response.body.match(%r{<meta name="?generator"? content="?(.*?)"?/?>}i)
